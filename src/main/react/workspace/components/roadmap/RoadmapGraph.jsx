...
const RoadmapGraph = ({
    ...
    useEffect(() => {
        setBlocksVisibility(getBloksVisibility(stageContainer.current, stageWrapper.current));
        console.log("asdasd");
    ...
    const moveGraph = (direction) => {
        ...
        if (pixelsToInt(stageContainer.current[0].style.right) >= 40 && direction === "right") {
            stageContainer.current[0].style.right = "40px";
            return;
        }
        if (pixelsToInt(stageContainer.current[1].style.right) <= 
            stageWrapper.current.offsetWidth - stageContainer.current[1].offsetWidth && 
            direction === "left") {
            return;
        }
        ...
        <div className={styles.graph}>
            ...
            <div className={styles.graphRoadmap} ref={stageWrapper}>
                {stages.map((stage, i) => {
                    ...
                    return (
                        <div className={styles.graphStage} style={{top: (180 * position) + "px", right: marginRight + "px"}} 
                             key={i} ref={(element) => stageContainer.current[i+1] = element}>
                ...
                <div className={[styles.moveGraphButton, styles.moveGraphButtonRight].join(" ")} onClick={() => moveGraph("right")}>

                </div>
                <div className={[styles.moveGraphButton, styles.moveGraphButtonLeft].join(" ")} onClick={() => moveGraph("left")}>
